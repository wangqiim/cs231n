...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        for i in range(2*N):
            if i != k:
                l1 += torch.exp(sim(z_k, out[i]) / tau)
            if i != k+N:
                l2 += torch.exp(sim(z_k_N, out[i]) / tau)
        l1 = torch.exp(sim(z_k, z_k_N) / tau) / l1
        l2 = torch.exp(sim(z_k_N, z_k) / tau) / l2
        total_loss += -torch.log(l1) - torch.log(l2)
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    pos_pairs = sim_positive_pairs(torch.cat([out_left, out_right], dim=0), torch.cat([out_right, out_left], dim=0))  # [N, 1]
    ...
    numerator = torch.exp(pos_pairs / tau)  # [2*N, 1]
    ...
    loss = numerator / denom  # [2*N, 1]
    loss = -torch.log(loss)  # [2*N, 1]
    loss = loss.mean()  # [1]
